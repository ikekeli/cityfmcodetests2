...
public partial class cityfmcodetests {
    ...
    {
        get => HttpData.Resolve<Product>("product");
        set => HttpData["product"] = value;
    }

    /// <summary>
    /// Table class for product
    /// </summary>
    public class Product : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<MySqlDbType> _name;

        public readonly DbField<MySqlDbType> description;

        public readonly DbField<MySqlDbType> unitPrice;

        public readonly DbField<MySqlDbType> maximumQuantity;
        ...
        public Product()
        {
            ...
            Name = "product";
            Type = "TABLE";
            UpdateTable = "product"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            productId = new (this, "x_productId", 200, MySqlDbType.VarString) {
                ...
                CustomMessage = Language.FieldPhrase("product", "productId", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("productId", productId);

            // name
            _name = new (this, "x__name", 200, MySqlDbType.VarString) {
                Name = "name",
                Expression = "`name`",
                UseBasicSearch = true,
                ...
                Required = true, // Required field
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY" },
                CustomMessage = Language.FieldPhrase("product", "_name", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("name", _name);

            // description
            description = new (this, "x_description", 200, MySqlDbType.VarString) {
                Name = "description",
                ...
                Required = true, // Required field
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY" },
                CustomMessage = Language.FieldPhrase("product", "description", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("description", description);

            // unitPrice
            unitPrice = new (this, "x_unitPrice", 5, MySqlDbType.Double) {
                Name = "unitPrice",
                Expression = "`unitPrice`",
                BasicSearchExpression = "`unitPrice`",
                DateTimeFormat = -1,
                VirtualExpression = "`unitPrice`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("product", "unitPrice", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("unitPrice", unitPrice);

            // maximumQuantity
            maximumQuantity = new (this, "x_maximumQuantity", 3, MySqlDbType.Int64) {
                Name = "maximumQuantity",
                Expression = "`maximumQuantity`",
                BasicSearchExpression = "`maximumQuantity`",
                DateTimeFormat = -1,
                VirtualExpression = "`maximumQuantity`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("product", "maximumQuantity", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("maximumQuantity", maximumQuantity);
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                unitPrice.DbValue = row["unitPrice"]; // Set DB value only
                maximumQuantity.DbValue = row["maximumQuantity"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "productview"))
                return Language.Phrase("View");
            else if (SameString(pageName, "productedit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "productadd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "ProductView",
                Config.ApiAddAction => "ProductAdd",
                Config.ApiEditAction => "ProductEdit",
                Config.ApiDeleteAction => "ProductDelete",
                Config.ApiListAction => "ProductList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "productlist";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("productview", parm);
            else
                url = KeyUrl("productview", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "productadd";
        ...
        {
            string url = "";
            url = KeyUrl("productedit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("productlist", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("productadd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("productlist", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("productdelete")); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            ...
            unitPrice.SetDbValue(dr["unitPrice"]);
            maximumQuantity.SetDbValue(dr["maximumQuantity"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "ProductList";
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(unitPrice);
                        doc.ExportCaption(maximumQuantity);
                    } else {
                        doc.ExportCaption(productId);
                        doc.ExportCaption(_name);
                        doc.ExportCaption(description);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(unitPrice);
                            await doc.ExportField(maximumQuantity);
                        } else {
                            await doc.ExportField(productId);
                            await doc.ExportField(_name);
                            await doc.ExportField(description);
