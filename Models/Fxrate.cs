...
public partial class cityfmcodetests {
    ...
    /// </summary>
    [MaybeNull]
    public static Fxrate fxrate
    {
        get => HttpData.Resolve<Fxrate>("fxrate");
        set => HttpData["fxrate"] = value;
    }

    /// <summary>
    /// Table class for fxrate
    /// </summary>
    public class Fxrate : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<MySqlDbType> id;

        public readonly DbField<MySqlDbType> sourceCurrency;

        public readonly DbField<MySqlDbType> targetCurrency;

        public readonly DbField<MySqlDbType> rate;
        ...
        public Fxrate()
        {
            ...
            Name = "fxrate";
            Type = "TABLE";
            UpdateTable = "fxrate"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            // id
            id = new (this, "x_id", 20, MySqlDbType.Int32) {
                Name = "id",
                Expression = "`id`",
                ...
                CustomMessage = Language.FieldPhrase("fxrate", "id", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("id", id);

            // sourceCurrency
            sourceCurrency = new (this, "x_sourceCurrency", 200, MySqlDbType.VarString) {
                Name = "sourceCurrency",
                Expression = "`sourceCurrency`",
                UseBasicSearch = true,
                BasicSearchExpression = "`sourceCurrency`",
                DateTimeFormat = -1,
                VirtualExpression = "`sourceCurrency`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("fxrate", "sourceCurrency", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("sourceCurrency", sourceCurrency);

            // targetCurrency
            targetCurrency = new (this, "x_targetCurrency", 200, MySqlDbType.VarString) {
                Name = "targetCurrency",
                Expression = "`targetCurrency`",
                UseBasicSearch = true,
                BasicSearchExpression = "`targetCurrency`",
                DateTimeFormat = -1,
                VirtualExpression = "`targetCurrency`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                InputTextType = "text",
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("fxrate", "targetCurrency", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("targetCurrency", targetCurrency);

            // rate
            rate = new (this, "x_rate", 5, MySqlDbType.Double) {
                Name = "rate",
                Expression = "`rate`",
                BasicSearchExpression = "`rate`",
                DateTimeFormat = -1,
                ...
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL" },
                CustomMessage = Language.FieldPhrase("fxrate", "rate", "CustomMsg"),
                IsUpload = false
            };
            rate.GetDefault = () => 0;
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                sourceCurrency.DbValue = row["sourceCurrency"]; // Set DB value only
                targetCurrency.DbValue = row["targetCurrency"]; // Set DB value only
                rate.DbValue = row["rate"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "fxrateview"))
                return Language.Phrase("View");
            else if (SameString(pageName, "fxrateedit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "fxrateadd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "FxrateView",
                Config.ApiAddAction => "FxrateAdd",
                Config.ApiEditAction => "FxrateEdit",
                Config.ApiDeleteAction => "FxrateDelete",
                Config.ApiListAction => "FxrateList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "fxratelist";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("fxrateview", parm);
            else
                url = KeyUrl("fxrateview", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "fxrateadd";
        ...
        {
            string url = "";
            url = KeyUrl("fxrateedit", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline edit URL
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("fxratelist", "action=edit"))); // DN
        ...
        {
            string url = "";
            url = KeyUrl("fxrateadd", parm);
            return AppPath(AddMasterUrl(url)); // DN
        }

        // Inline copy URL
        public string InlineCopyUrl =>
            AppPath(AddMasterUrl(KeyUrl("fxratelist", "action=copy"))); // DN

        // Delete URL
        public string DeleteUrl => UseAjaxActions && Param<bool>("infinitescroll") && CurrentPageID() == "list"
            ? AppPath(KeyUrl(Config.ApiUrl + Config.ApiDeleteAction + "/" + TableVar))
            : AppPath(KeyUrl("fxratedelete")); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            if (dr == null)
                return;
            id.SetDbValue(dr["id"]);
            sourceCurrency.SetDbValue(dr["sourceCurrency"]);
            targetCurrency.SetDbValue(dr["targetCurrency"]);
            rate.SetDbValue(dr["rate"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "FxrateList";
        ...
        public async Task RenderListRow()
        {
            ...
            sourceCurrency.TooltipValue = "";

            // targetCurrency
            targetCurrency.HrefValue = "";
            targetCurrency.TooltipValue = "";
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(sourceCurrency);
                        doc.ExportCaption(targetCurrency);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(sourceCurrency);
                            await doc.ExportField(targetCurrency);
