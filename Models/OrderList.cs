...
public partial class cityfmcodetests {
    ...
    {
        get => HttpData.Get<OrderList>("orderList")!;
        set => HttpData["orderList"] = value;
    }

    /// <summary>
    /// Page class for order
    /// </summary>
    public class OrderList : OrderListBase
    {
        // Constructor
        public OrderList(Controller controller) : base(controller)
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class OrderListBase : Order
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "orderList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "forderlist";
        ...
        public OrderListBase()
        {
            ...
            MultiDeleteUrl = "orderdelete";
            MultiUpdateUrl = "orderupdate";
        ...
        // Page name
        public string PageName => "orderlist";
        ...
        // Constructor
        public OrderListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "orderview" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(customerName.AdvancedSearch.ToJson())); // Field customerName
            filters.Merge(JObject.Parse(customerEmail.AdvancedSearch.ToJson())); // Field customerEmail
            filters.Merge(JObject.Parse(lineItems.AdvancedSearch.ToJson())); // Field lineItems
        ...
        protected bool RestoreFilterList() {
            ...
            // Field customerName
            if (filter?.TryGetValue("x_customerName", out sv) ?? false) {
                customerName.AdvancedSearch.SearchValue = sv;
                customerName.AdvancedSearch.SearchOperator = filter["z_customerName"];
                customerName.AdvancedSearch.SearchCondition = filter["v_customerName"];
                customerName.AdvancedSearch.SearchValue2 = filter["y_customerName"];
                customerName.AdvancedSearch.SearchOperator2 = filter["w_customerName"];
                customerName.AdvancedSearch.Save();
            }

            // Field customerEmail
            if (filter?.TryGetValue("x_customerEmail", out sv) ?? false) {
                customerEmail.AdvancedSearch.SearchValue = sv;
                customerEmail.AdvancedSearch.SearchOperator = filter["z_customerEmail"];
                customerEmail.AdvancedSearch.SearchCondition = filter["v_customerEmail"];
                customerEmail.AdvancedSearch.SearchValue2 = filter["y_customerEmail"];
                customerEmail.AdvancedSearch.SearchOperator2 = filter["w_customerEmail"];
                customerEmail.AdvancedSearch.Save();
            }

            // Field lineItems
            if (filter?.TryGetValue("x_lineItems", out sv) ?? false) {
                lineItems.AdvancedSearch.SearchValue = sv;
                lineItems.AdvancedSearch.SearchOperator = filter["z_lineItems"];
                lineItems.AdvancedSearch.SearchCondition = filter["v_lineItems"];
                lineItems.AdvancedSearch.SearchValue2 = filter["y_lineItems"];
                lineItems.AdvancedSearch.SearchOperator2 = filter["w_lineItems"];
                lineItems.AdvancedSearch.Save();
            }
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(customerName);
            searchFlds.Add(customerEmail);
            searchFlds.Add(lineItems);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(customerName); // customerName
                UpdateSort(customerEmail); // customerEmail
                UpdateSort(lineItems); // lineItems
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""order"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"forderlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"forderlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""order"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("customerName")); // DN
                CreateColumnOption(option.Add("customerEmail")); // DN
                CreateColumnOption(option.Add("lineItems")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fordersrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fordersrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"forderlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(orderList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(orderList.RowCount) + "_order");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(orderList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && orderList.RowType == RowType.Add || IsEdit && orderList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fordersrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
