...
public partial class cityfmcodetests {
    ...
    {
        get => HttpData.Resolve<Order>("order");
        set => HttpData["order"] = value;
    }

    /// <summary>
    /// Table class for order
    /// </summary>
    public class Order : DbTable, IQueryFactory
    {
        ...
        public readonly DbField<MySqlDbType> customerID;

        public readonly DbField<MySqlDbType> customerName;

        public readonly DbField<MySqlDbType> customerEmail;

        public readonly DbField<MySqlDbType> lineItems;
        ...
        public Order()
        {
            ...
            Name = "order";
            Type = "TABLE";
            UpdateTable = "order"; // Update Table
            DbId = "DB"; // DN
            ExportAll = true;
            ...
            // customerID
            customerID = new (this, "x_customerID", 20, MySqlDbType.Int32) {
                Name = "customerID",
                Expression = "`customerID`",
                BasicSearchExpression = "`customerID`",
                DateTimeFormat = -1,
                ...
                CustomMessage = Language.FieldPhrase("order", "customerID", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("customerID", customerID);

            // customerName
            customerName = new (this, "x_customerName", 200, MySqlDbType.VarString) {
                Name = "customerName",
                Expression = "`customerName`",
                UseBasicSearch = true,
                BasicSearchExpression = "`customerName`",
                DateTimeFormat = -1,
                VirtualExpression = "`customerName`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                CustomMessage = Language.FieldPhrase("order", "customerName", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("customerName", customerName);

            // customerEmail
            customerEmail = new (this, "x_customerEmail", 200, MySqlDbType.VarString) {
                Name = "customerEmail",
                Expression = "`customerEmail`",
                UseBasicSearch = true,
                BasicSearchExpression = "`customerEmail`",
                DateTimeFormat = -1,
                VirtualExpression = "`customerEmail`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                Required = true, // Required field
                SearchOperators = new () { "=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY" },
                CustomMessage = Language.FieldPhrase("order", "customerEmail", "CustomMsg"),
                IsUpload = false
            };
            Fields.Add("customerEmail", customerEmail);

            // lineItems
            lineItems = new (this, "x_lineItems", 200, MySqlDbType.VarString) {
                Name = "lineItems",
                Expression = "`lineItems`",
                UseBasicSearch = true,
                BasicSearchExpression = "`lineItems`",
                DateTimeFormat = -1,
                VirtualExpression = "`lineItems`",
                IsVirtual = false,
                ForceSelection = false,
                SelectMultiple = false,
                VirtualSearch = false,
                ...
                SearchOperators = new () { "=", "<>" },
                CustomMessage = Language.FieldPhrase("order", "lineItems", "CustomMsg"),
                IsUpload = false
            };
            lineItems.Lookup = new Lookup<DbField>("lineItems", "product", false, "productId", new List<string> {"name", "unitPrice", "", ""}, "", "", new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, new List<string> {}, "", "", "CONCAT(COALESCE(`name`, ''),'" + ValueSeparator(1, lineItems) + "',COALESCE(`unitPrice`,''))");
            Fields.Add("lineItems", lineItems);
        ...
        public void LoadDbValues(Dictionary<string, object>? row)
        {
            ...
            try {
                ...
                customerName.DbValue = row["customerName"]; // Set DB value only
                customerEmail.DbValue = row["customerEmail"]; // Set DB value only
                lineItems.DbValue = row["lineItems"]; // Set DB value only
            } catch {}
        }
        ...
        {
            if (SameString(pageName, "orderview"))
                return Language.Phrase("View");
            else if (SameString(pageName, "orderedit"))
                return Language.Phrase("Edit");
            else if (SameString(pageName, "orderadd"))
        ...
        public string GetApiPageName(string action)
        {
            return action.ToLowerInvariant() switch {
                Config.ApiViewAction => "OrderView",
                Config.ApiAddAction => "OrderAdd",
                Config.ApiEditAction => "OrderEdit",
                Config.ApiDeleteAction => "OrderDelete",
                Config.ApiListAction => "OrderList",
                _ => String.Empty
            };
        }
        ...
        // List URL
        public string ListUrl => "orderlist";
        ...
        public string GetViewUrl(string parm = "")
        {
            string url = "";
            if (!Empty(parm))
                url = KeyUrl("orderview", parm);
            else
                url = KeyUrl("orderview", Config.TableShowDetail + "=");
            return AddMasterUrl(url);
        }

        // Add URL
        public string AddUrl { get; set; } = "orderadd";
        ...
        public string InlineEditUrl =>
            AppPath(AddMasterUrl(KeyUrl("orderlist", "action=edit"))); // DN
        ...
        public void LoadListRowValues(DbDataReader? dr)
        {
            ...
            customerName.SetDbValue(dr["customerName"]);
            customerEmail.SetDbValue(dr["customerEmail"]);
            lineItems.SetDbValue(dr["lineItems"]);
        }

        // Render list content
        public async Task<string> RenderListContent(string filter)
        {
            string pageName = "OrderList";
        ...
        public async Task RenderEditRow()
        {
            ...
            // customerName
            customerName.SetupEditAttributes();
            if (!customerName.Raw)
                customerName.CurrentValue = HtmlDecode(customerName.CurrentValue);
            customerName.EditValue = HtmlEncode(customerName.CurrentValue);
            customerName.PlaceHolder = RemoveHtml(customerName.Caption);

            // customerEmail
            customerEmail.SetupEditAttributes();
            if (!customerEmail.Raw)
                customerEmail.CurrentValue = HtmlDecode(customerEmail.CurrentValue);
            customerEmail.EditValue = HtmlEncode(customerEmail.CurrentValue);
            customerEmail.PlaceHolder = RemoveHtml(customerEmail.Caption);

            // lineItems
            lineItems.SetupEditAttributes();
            lineItems.PlaceHolder = RemoveHtml(lineItems.Caption);
            ...
            if (!doc.ExportCustom) {
                ...
                if (doc.Horizontal) { // Horizontal format, write header
                    ...
                    if (exportType == "view") {
                        ...
                        doc.ExportCaption(customerName);
                        doc.ExportCaption(customerEmail);
                        doc.ExportCaption(lineItems);
                    } else {
                        doc.ExportCaption(customerID);
                        doc.ExportCaption(customerName);
                        doc.ExportCaption(customerEmail);
                ...
                if (recCnt >= startRec) {
                    ...
                    if (!doc.ExportCustom) {
                        ...
                        if (exportType == "view") {
                            ...
                            await doc.ExportField(customerName);
                            await doc.ExportField(customerEmail);
                            await doc.ExportField(lineItems);
                        } else {
                            await doc.ExportField(customerID);
                            await doc.ExportField(customerName);
                            await doc.ExportField(customerEmail);
