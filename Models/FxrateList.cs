...
public partial class cityfmcodetests {
    ...
    /// fxrateList
    /// </summary>
    public static FxrateList fxrateList
    {
        get => HttpData.Get<FxrateList>("fxrateList")!;
        set => HttpData["fxrateList"] = value;
    }

    /// <summary>
    /// Page class for fxrate
    /// </summary>
    public class FxrateList : FxrateListBase
    {
        // Constructor
        public FxrateList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public FxrateList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class FxrateListBase : Fxrate
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "fxrateList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "ffxratelist";
        ...
        public FxrateListBase()
        {
            ...
            MultiDeleteUrl = "fxratedelete";
            MultiUpdateUrl = "fxrateupdate";
        ...
        // Page name
        public string PageName => "fxratelist";
        ...
        // Constructor
        public FxrateListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(sourceCurrency.AdvancedSearch.ToJson())); // Field sourceCurrency
            filters.Merge(JObject.Parse(targetCurrency.AdvancedSearch.ToJson())); // Field targetCurrency
        ...
        protected bool RestoreFilterList() {
            ...
            if (filter?.TryGetValue("x_sourceCurrency", out sv) ?? false) {
                sourceCurrency.AdvancedSearch.SearchValue = sv;
                sourceCurrency.AdvancedSearch.SearchOperator = filter["z_sourceCurrency"];
                sourceCurrency.AdvancedSearch.SearchCondition = filter["v_sourceCurrency"];
                sourceCurrency.AdvancedSearch.SearchValue2 = filter["y_sourceCurrency"];
                sourceCurrency.AdvancedSearch.SearchOperator2 = filter["w_sourceCurrency"];
                sourceCurrency.AdvancedSearch.Save();
            }

            // Field targetCurrency
            if (filter?.TryGetValue("x_targetCurrency", out sv) ?? false) {
                targetCurrency.AdvancedSearch.SearchValue = sv;
                targetCurrency.AdvancedSearch.SearchOperator = filter["z_targetCurrency"];
                targetCurrency.AdvancedSearch.SearchCondition = filter["v_targetCurrency"];
                targetCurrency.AdvancedSearch.SearchValue2 = filter["y_targetCurrency"];
                targetCurrency.AdvancedSearch.SearchOperator2 = filter["w_targetCurrency"];
                targetCurrency.AdvancedSearch.Save();
            }

            // Field rate
            if (filter?.TryGetValue("x_rate", out sv) ?? false) {
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(sourceCurrency);
            searchFlds.Add(targetCurrency);
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(id); // id
                UpdateSort(sourceCurrency); // sourceCurrency
                UpdateSort(targetCurrency); // targetCurrency
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""fxrate"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""fxrate"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ffxratelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"ffxratelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""fxrate"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("sourceCurrency")); // DN
                CreateColumnOption(option.Add("targetCurrency")); // DN
                CreateColumnOption(option.Add("rate")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"ffxratesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"ffxratesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"ffxratelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(fxrateList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(fxrateList.RowCount) + "_fxrate");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(fxrateList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && fxrateList.RowType == RowType.Add || IsEdit && fxrateList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"ffxratesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
