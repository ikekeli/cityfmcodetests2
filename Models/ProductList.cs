...
public partial class cityfmcodetests {
    ...
    {
        get => HttpData.Get<ProductList>("productList")!;
        set => HttpData["productList"] = value;
    }

    /// <summary>
    /// Page class for product
    /// </summary>
    public class ProductList : ProductListBase
    {
        // Constructor
        public ProductList(Controller controller) : base(controller)
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class ProductListBase : Product
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "product";

        // Page object name
        public string PageObjName = "productList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fproductlist";
        ...
        public ProductListBase()
        {
            ...
            MultiDeleteUrl = "productdelete";
            MultiUpdateUrl = "productupdate";
        ...
        // Page name
        public string PageName => "productlist";
        ...
        public void SetVisibility()
        {
            ...
            unitPrice.SetVisibility();
            maximumQuantity.SetVisibility();
        }

        // Constructor
        public ProductListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "productview" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected string GetRecordKeyValue(Dictionary<string, object> dict) {
            string key = "";
            key += UrlEncode(ConvertToString(dict.ContainsKey("productId") ? dict["productId"] : productId.CurrentValue));
            return key;
        }
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(productId.AdvancedSearch.ToJson())); // Field productId
            ...
            filters.Merge(JObject.Parse(unitPrice.AdvancedSearch.ToJson())); // Field unitPrice
            filters.Merge(JObject.Parse(maximumQuantity.AdvancedSearch.ToJson())); // Field maximumQuantity
        ...
        protected bool RestoreFilterList() {
            ...
            // Field productId
            if (filter?.TryGetValue("x_productId", out sv) ?? false) {
                productId.AdvancedSearch.SearchValue = sv;
                productId.AdvancedSearch.SearchOperator = filter["z_productId"];
                productId.AdvancedSearch.SearchCondition = filter["v_productId"];
                productId.AdvancedSearch.SearchValue2 = filter["y_productId"];
                productId.AdvancedSearch.SearchOperator2 = filter["w_productId"];
                productId.AdvancedSearch.Save();
            }
            ...
            // Field unitPrice
            if (filter?.TryGetValue("x_unitPrice", out sv) ?? false) {
                unitPrice.AdvancedSearch.SearchValue = sv;
                unitPrice.AdvancedSearch.SearchOperator = filter["z_unitPrice"];
                unitPrice.AdvancedSearch.SearchCondition = filter["v_unitPrice"];
                unitPrice.AdvancedSearch.SearchValue2 = filter["y_unitPrice"];
                unitPrice.AdvancedSearch.SearchOperator2 = filter["w_unitPrice"];
                unitPrice.AdvancedSearch.Save();
            }

            // Field maximumQuantity
            if (filter?.TryGetValue("x_maximumQuantity", out sv) ?? false) {
                maximumQuantity.AdvancedSearch.SearchValue = sv;
                maximumQuantity.AdvancedSearch.SearchOperator = filter["z_maximumQuantity"];
                maximumQuantity.AdvancedSearch.SearchCondition = filter["v_maximumQuantity"];
                maximumQuantity.AdvancedSearch.SearchValue2 = filter["y_maximumQuantity"];
                maximumQuantity.AdvancedSearch.SearchOperator2 = filter["w_maximumQuantity"];
                maximumQuantity.AdvancedSearch.Save();
            }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(productId); // productId
                UpdateSort(_name); // name
                UpdateSort(description); // description
                UpdateSort(unitPrice); // unitPrice
                UpdateSort(maximumQuantity); // maximumQuantity
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""product"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""product"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fproductlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fproductlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
            ...
            listOption?.SetBody("<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" + RowCount + "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" + HtmlEncode(productId.CurrentValue) + "\" data-ew-action=\"select-key\"></div>");
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""product"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("productId")); // DN
                CreateColumnOption(option.Add("name")); // DN
                CreateColumnOption(option.Add("description")); // DN
                CreateColumnOption(option.Add("unitPrice")); // DN
                CreateColumnOption(option.Add("maximumQuantity")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fproductsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fproductsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fproductlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(productList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(productList.RowCount) + "_product");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(productList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && productList.RowType == RowType.Add || IsEdit && productList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            RowSelected(row);
            productId.SetDbValue(row["productId"]);
            _name.SetDbValue(row["name"]);
            description.SetDbValue(row["description"]);
            unitPrice.SetDbValue(row["unitPrice"]);
            maximumQuantity.SetDbValue(row["maximumQuantity"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            var row = new Dictionary<string, object>();
            row.Add("productId", productId.DefaultValue ?? DbNullValue); // DN
            row.Add("name", _name.DefaultValue ?? DbNullValue); // DN
            row.Add("description", description.DefaultValue ?? DbNullValue); // DN
            row.Add("unitPrice", unitPrice.DefaultValue ?? DbNullValue); // DN
            row.Add("maximumQuantity", maximumQuantity.DefaultValue ?? DbNullValue); // DN
            return row;
        }

        #pragma warning disable 618, 1998
        // Load old record
        protected async Task<Dictionary<string, object>?> LoadOldRecord(DatabaseConnectionBase<MySqlConnection, MySqlCommand, MySqlDataReader, MySqlDbType>? cnn = null) {
            // Load old record
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // unitPrice
                unitPrice.ViewValue = unitPrice.CurrentValue;
                unitPrice.ViewValue = FormatNumber(unitPrice.ViewValue, unitPrice.FormatPattern);
                unitPrice.ViewCustomAttributes = "";

                // maximumQuantity
                maximumQuantity.ViewValue = maximumQuantity.CurrentValue;
                maximumQuantity.ViewValue = FormatNumber(maximumQuantity.ViewValue, maximumQuantity.FormatPattern);
                maximumQuantity.ViewCustomAttributes = "";
                ...
                // unitPrice
                unitPrice.HrefValue = "";
                unitPrice.TooltipValue = "";

                // maximumQuantity
                maximumQuantity.HrefValue = "";
                maximumQuantity.TooltipValue = "";
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fproductsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
